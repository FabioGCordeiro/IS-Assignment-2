 ...
 protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    if((Integer.parseInt(request.getParameter("order")) == 0)){
      ...
      categoryItems.sort(Comparator.comparing(Item::getInsertionDate).thenComparing(Item::getId));
        ...
        categoryItems.sort(Comparator.comparing(Item::getInsertionDate).reversed().thenComparing(Item::getId));
    }
    ...
    out.println("<form action=SearchByCategory><input type=hidden name=order value=0></input><input type=hidden name=category value="+category+"></input><button type=submit> Older To Recent </button><br></form><form action=SearchByCategory><input type=hidden name=order value=1></input><input type=hidden name=category value="+category+"></input><button type=submit> Recent To Older </button><br><br></form>");
    out.println("</div>");

    //PRICE ORDERING BUTTONS
    out.println("<div style=position:absolute;top:100px;right:100px>Order By Price:");
    out.println("<form action=SearchByCategory><input type=hidden name=order value=2></input><input type=hidden name=category value="+category+"></input><button type=submit> Cheapest To Most Expensive </button><br></form><form action=SearchByCategory><input type=hidden name=order value=3></input><input type=hidden name=category value="+category+"></input><button type=submit> Most Expensive To Cheapest  </button><br><br></form>");
    out.println("</div>");

    //NAME ORDERING BUTTONS
    out.println("<div style=position:absolute;top:190px;right:188px>Order By Price:");
    out.println("<form action=SearchByCategory><input type=hidden name=order value=4></input></input><input type=hidden name=category value="+category+"></input><button type=submit> A to Z </button><br></form><form action=SearchByCategory><input type=hidden name=order value=5></input><input type=hidden name=category value="+category+"></input><button type=submit> Z to A  </button><br><br></form>");
    out.println("</div>");
